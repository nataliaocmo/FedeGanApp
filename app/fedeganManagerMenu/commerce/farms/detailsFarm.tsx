...
export default function DetailsFarm() {
    ...
    useEffect(() => {
        ...
        // Load farm name
        const farmDocRef = doc(db, "farms", fincaId);
        getDoc(farmDocRef).then((docSnap) => {
            if (docSnap.exists()) {
                setFarmName(docSnap.data().name || "Finca sin nombre");
                console.log("Nombre de finca:", docSnap.data().name);
            } else {
                setFarmName("Finca no encontrada");
                console.error("Finca no encontrada:", fincaId);
            }
        }).catch((error) => {
            console.error("Error al cargar nombre de finca:", error.message, { code: error.code });
            Alert.alert("Error", `No se pudo cargar el nombre de la finca: ${error.message}`);
        });
    ...
    const totalImports = movements.filter((m) => m.type === "import").length;
    const totalExports = movements.filter((m) => m.type === "export").length;
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptyText}>No hay movimientos registrados para esta finca.</Text>
                        </View>
                    }
                />
