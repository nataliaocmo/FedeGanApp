...
export default function DetailCampaigns() {
    ...
    const [farm, setFarm] = useState<{ name: string; region: string }>({ name: "Finca desconocida", region: "Colombia" });
    ...
    useEffect(() => {
        ...
        // Cargar la campaña
        const campaignRef = doc(db, "campaigns", campaignId as string); // Type assertion safe after !campaignId check
        const unsubscribeCampaign = onSnapshot(campaignRef, (docSnapshot) => {
            if (docSnapshot.exists()) {
                const data = docSnapshot.data() as CampaignData;
                console.log("Campaign data from Firestore:", data);
                setCampaign({ ...data, id: docSnapshot.id });
                console.log("Campaña cargada:", { ...data, id: docSnapshot.id });
            } else {
        ...
        const unsubscribeFarms = onSnapshot(qFarms, (snapshot) => {
            const farmsData = snapshot.docs.reduce((acc, doc) => {
                ...
                return {
                    ...acc,
                    [doc.id]: { name: data.name || "Finca desconocida", region: data.region || "Colombia" },
                };
            }, {} as { [key: string]: { name: string; region: string } });
            if (campaign?.farmId) {
                setFarm(farmsData[campaign.farmId] || { name: "Finca desconocida", region: "Colombia" });
            }
        });
    ...
    const renderVaccinationRecord = ({ item }: { item: NonNullable<Campaign["vaccinationRecords"]>[number] }) => (
        <View style={styles.recordItem}>
            <View style={styles.detailRow}>
                <Icon name="pill" size={16} color={COLORS.darkGray} style={styles.detailIcon} />
                <Text style={styles.recordText}>Vacunados: {item.vaccinatedAnimals}</Text>
            </View>
            <View style={styles.detailRow}>
                <Icon name="calendar" size={16} color={COLORS.darkGray} style={styles.detailIcon} />
                <Text style={styles.recordText}>Fecha: {new Date(item.vaccinationDate).toLocaleDateString()}</Text>
            </View>
            <View style={styles.detailRow}>
                <Icon name="comment" size={16} color={COLORS.darkGray} style={styles.detailIcon} />
                <Text style={styles.recordText}>Comentarios: {item.comments}</Text>
            </View>
            <View style={styles.detailRow}>
                <Icon name="account" size={16} color={COLORS.darkGray} style={styles.detailIcon} />
                <Text style={styles.recordText}>Creado por: {item.createdBy}</Text>
            </View>
        </View>
    );
        ...
        <View style={styles.container}>
            ...
            <View style={styles.campaignDetails}>
                ...
                <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailText}>Finca: {farm.name}</Text>
                </View>
                <View style={styles.detailRow}>
                    <Icon name="map-marker" size={20} color={COLORS.forestGreen} />
                    <Text style={styles.detailText}>Región: {farm.region}</Text>
                </View>
                <View style={styles.detailRow}>
            ...
            <View style={styles.recordsContainer}>
                <Text style={styles.recordsTitle}>Registros de Vacunación</Text>
                <FlatList
                    data={(campaign.vaccinationRecords ?? []) as NonNullable<Campaign["vaccinationRecords"]>}
                    renderItem={renderVaccinationRecord}
                    keyExtractor={(item, index) => `${item.createdAt}-${index}`}
                    showsVerticalScrollIndicator={false}
                    ListEmptyComponent={<Text style={styles.emptyRecordsText}>No hay registros de vacunación</Text>}
                />
            </View>
            {user?.role === "vaccinationAgent" && (
