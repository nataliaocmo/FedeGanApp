...
export default function BrotesMenu() {
    const router = useRouter();

    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Icon name="flask-outline" size={40} color={COLORS.forestGreen} />
                <Text style={styles.title}>Brotes</Text>
                <Text style={styles.subtitle}>Elige una opción:</Text>
            </View>
            <TouchableOpacity
                style={styles.button}
                onPress={() => router.push("/fedeganManagerMenu/outbreaks/interactiveMap")}
                activeOpacity={0.7}
            >
                <Icon name="plus-circle-outline" size={24} color={COLORS.white} style={styles.icon} />
                <Text style={styles.buttonText}>Ver mapa de brotes</Text>
            </TouchableOpacity>
            <TouchableOpacity
                style={styles.button}
                onPress={() => router.push("/fedeganManagerMenu/outbreaks/exportInformation")}
                activeOpacity={0.7}
            >
